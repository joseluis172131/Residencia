...
class Oficina_AdminController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        // Generar automáticamente el código (con prefijo PROF- y número secuencial)
        // Obtener el último código registrado en la base de datos
        $ultimoCodigo = Oficina_AdminModel::where('codigo', 'like', 'PROF-%')
                                   ->orderByDesc('codigo')
                                   ->first();
    
        // Si no hay registros previos, empezar con PROF-001
        if ($ultimoCodigo) {
            // Extraer el número del último código (quitar el prefijo 'PROF-')
            $numeroSecuencial = (int) substr($ultimoCodigo->codigo, 5);  // "PROF-" tiene 5 caracteres
        ...
        $nuevoCodigo = 'PROF-' . str_pad($nuevoNumeroSecuencial, 3, '0', STR_PAD_LEFT);
    ...
    public function destroy($id)
    {
        try {
            ...
            $oficina = Oficina_AdminModel::findOrFail($id);
            $codigoEliminar = $oficina->codigo;  // Usar el código correcto
